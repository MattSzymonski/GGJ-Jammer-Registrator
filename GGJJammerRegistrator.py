...
def errorClearWindow(text):
	...
	adminUsernameEntry.config(state=NORMAL)
	adminPasswordEntry.config(state=NORMAL)
...
def setup():
	...
	if(len(siteAddress.get()) == 0 or len(adminUsername.get()) == 0 or len(adminPassword.get()) == 0 or jammersListFilename == None):
	...
	try:
		...
		try:
			driver = webdriver.Firefox(log_path='NUL', service_log_path='NUL', executable_path='geckodriver.exe')
		...
		logPrinter("Logging in")
		loginButton = driver.find_element_by_xpath("//a[@href='/user']")
		loginButton.click()
		time.sleep(1)
		username = driver.find_element_by_xpath("//input[@id='edit-name']")
		username.clear()
		username.send_keys(adminUsername.get())
		time.sleep(0.5)
		password = driver.find_element_by_xpath("//input[@id='edit-pass']")
		password.clear()
		password.send_keys(adminPassword.get())
...
adminUsernameLabel = Label(window, text="Admin username or email", width=30, fg='white', bg='#585858', justify=CENTER)
adminUsernameLabel.place(x=45,y=60)
adminUsernameEntry =  Entry(window, textvar=adminUsername, width=30)
adminUsernameEntry.place(x=60,y=85)

adminPasswordLabel = Label(window, text="Admin password", width=30, fg='white', bg='#585858', justify=CENTER)
adminPasswordLabel.place(x=45,y=110)
adminPasswordEntry =  Entry(window, show="•", textvar=adminPassword, width=30)
adminPasswordEntry.place(x=60,y=135)

jammersLabel = Label(window, text="Jammers to register (.csv)", width=30, fg='white', bg='#585858', justify=CENTER)
jammersLabel.place(x=45,y=185)
jammersListLabel = Label(window, text="No file choosen", width=12, bg='#585858', fg="red")
jammersListLabel.place(x=150,y=212)
jammersChooseFileButton = Button(window, text="Choose file", relief=FLAT, command=chooseJammersListFile);
jammersChooseFileButton.place(x=60, y=210)
